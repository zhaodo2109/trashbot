...
class Servo:
    ...
    def reset_all(self, target_angle=90, speed='mid'):
        ...
        for ch in self.servo_map.keys():
            self.move_to(ch, target_angle, speed)
        self.save_state()
        print("[RESET] All servos centered at 90°.")

    # ---------- Execute preset ----------
    def run_preset(self, preset_name, speed='slow'):
        """Run a predefined servo position set."""
        if preset_name not in PRESETS:
            print(f"[ERROR] Preset '{preset_name}' not found.")
            return

        print(f"[PRESET] Executing '{preset_name}' (speed={speed}) ...")
        for ch, angle in PRESETS[preset_name]:
